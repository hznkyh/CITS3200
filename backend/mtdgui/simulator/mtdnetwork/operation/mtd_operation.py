...
class MTDOperation:
    ...
    def _mtd_trigger_action(self):
        ...
        while True:
            ...
            logger.info('MTD: %s triggered %.1fs' % (mtd.get_name(), self.env.now + self._proceed_time))
            ...
            if len(resource.users) == 0:
                ...
                if mtd.get_priority() not in self.network.get_suspended_mtd():
                    ...
                    logger.info('MTD: %s suspended at %.1fs due to resource occupation' %
    ...
    def _mtd_execute_action(self, env, mtd):
        ...
        logger.info('MTD: %s deployed in the network at %.1fs.' % (mtd.get_name(), start_time))
        ...
        logger.info('MTD: %s finished in %.1fs at %.1fs.' % (mtd.get_name(), duration, finish_time))
        # release resource
