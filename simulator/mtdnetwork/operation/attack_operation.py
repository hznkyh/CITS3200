...
class AttackOperation:
    def __init__(self, env, end_event, adversary, callback=None, proceed_time=0):
        """
    ...
    def _execute_attack_action(self, time, attack_action):
        ...
        try:
            logger.info("Adversary: Start %s at %.1fs." % (self.adversary.get_curr_process(), start_time))
        ...
        logger.info("Adversary: Processed %s at %.1fs." % (self.adversary.get_curr_process(), finish_time))
    ...
    def _handle_interrupt(self, start_time, name):
        ...
        if self._interrupted_mtd.get_resource_type() == 'network':
            ...
            logger.info('Adversary: Restarting with SCAN_HOST at %.1fs!' % (self.env.now + self._proceed_time))
            self._scan_host()
        elif self._interrupted_mtd.get_resource_type() == 'application':
            self._interrupted_mtd = None
            logger.info('Adversary: Restarting with SCAN_PORT at %.1fs!' % (self.env.now + self._proceed_time))
            self._scan_port()
    ...
    def _execute_scan_host(self):
        ...
        if len(adversary.get_host_stack()) > 0:
            ...
            logger.info("Adversary: Cannot discover new hosts!")
            return
